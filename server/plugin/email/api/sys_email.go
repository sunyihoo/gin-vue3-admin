...
import (
	...
	email_response "github.com/sunyihoo/gin-vue3-admin/server/plugin/email/model/response"
	"github.com/sunyihoo/gin-vue3-admin/server/plugin/email/service"
	"go.uber.org/zap"
)
...
func (s *EmailApi) SendEmail(c *gin.Context) {
	...
	if err != nil {
		global.GVA_LOG.Error("发送是失败！", zap.Error(err))
		response.FailWithMessage("发送失败", c)
		return
	}
	response.OkWithMessage("发送成功！", c)
